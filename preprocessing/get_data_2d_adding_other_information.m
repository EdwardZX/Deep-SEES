...
function y = get_slice_adding_value(adding,window,is_del_spatial,varargin)
y = arrayfun(@(m) get_single_slices(adding{m},window,is_del_spatial),...
    (1:numel(adding))','UniformOutput',false);
end
...
function y = adding_pos(pos,adding,is_del_iso)
%pos cell adding:double
sz = min(numel(pos),numel(adding));
pos = pos(1:sz); adding = mat2cell(adding(1:sz,:),ones(sz,1));
if is_del_iso
    y = arrayfun(@(x,y) [x{:},y{:}],pos,adding,'UniformOutput',false);
else
    y = arrayfun(@(x,y) [x{:},zeros(size(y{:}))],pos,adding,'UniformOutput',false);
end
end
